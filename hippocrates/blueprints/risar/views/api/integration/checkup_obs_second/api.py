...
from hippocrates.blueprints.risar.views.api.integration.checkup_obs_second.xform import \
    CheckupObsSecondXForm, CheckupObsSecondTicket25XForm
...
# @module.route('/api/integration/<int:api_version>/card/<int:card_id>/checkup/obs/second/<int:exam_obs_id>/', methods=['GET'])
# @api_method(hook=hook)
# def api_checkup_obs_second_get(api_version, card_id, exam_obs_id=None):
#     xform = CheckupObsSecondXForm(api_version)
#     xform.check_params(exam_obs_id, card_id)
#     xform.load_data()
#     return xform.as_json()
...
@module.route('/api/integration/<int:api_version>/card/<int:card_id>/checkup/obs/second/<int:exam_obs_id>/ticket25')
@api_method(hook=hook)
...
@module.route('/api/integration/<int:api_version>/card/<int:card_id>/checkup/obs/second/<int:exam_obs_id>/ticket25', methods=['PUT'])
@api_method(hook=hook)
def api_checkup_obs_second_ticket25_save(api_version, card_id, exam_obs_id):
    data = request.get_json()
    xform = CheckupObsSecondTicket25XForm(api_version)
    xform.check_params(exam_obs_id, card_id)
    xform.update_target_obj(data)
