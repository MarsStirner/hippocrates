...
from test_data import (pc_data)


def create_checkup(session, card_id, data):
    url = u'/risar/api/integration/0/card/%s/checkup/pc/' % card_id
    result = make_api_request('post', url, session, data)
    return result


def edit_checkup(session, card_id, concilium_id, data):
    url = u'/risar/api/integration/0/card/%s/checkup/pc/%s/' % (card_id, concilium_id)
    result = make_api_request('put', url, session, data)
    return result


def delete_checkup(session, card_id, checkup_id):
    url = u'/risar/api/integration/0/card/%s/checkup/pc/%s/' % (card_id, checkup_id)
    result = make_api_request('delete', url, session)
    return result


def test_register_edit_delete_pc_checkup(card_id):
    with make_login() as session:
        result = create_checkup(session, card_id, pc_data)
        checkup = result['result']
        checkup_id = checkup['exam_pc_id']
        ...
        result = edit_checkup(session, card_id, checkup_id, pc_data)
        checkup = result['result']
        meta = result['meta']
