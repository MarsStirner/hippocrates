...
def new_checkup(token, session_token, event_id):
    url = u'%s/risar/api/integration/0/card/%s/checkup/obs/pc/' % (mis_url, event_id)
    result = requests.post(
        url,
        json=test_pc_data,
        cookies={auth_token_name: token,
...
def change_checkup(token, session_token, event_id, checkup_id):
    url = u'%s/risar/api/integration/0/card/%s/checkup/obs/pc/%s/' % (mis_url, event_id, checkup_id)
    result = requests.put(
        url,
        json=test_pc_data,
        cookies={auth_token_name: token,
