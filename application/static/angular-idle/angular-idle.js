      ...
      function($interval, $log, $rootScope, $document, Keepalive) {
        ...
        var svc = {
          ...
          setTracking: function(on) {
            if (on) {
              $document.find('body').on(options.interrupt, this._onUserAction);
            } else {
              $document.find('body').off(options.interrupt, this._onUserAction);
            }
          }
        };

        svc.setTracking(true);
...
angular.module('ngIdle.title', [])
  .factory('IdleTitle', ['$document', '$interpolate', function($document, $interpolate) {
  ...
  .directive('idleTitle', ['IdleTitle', function(IdleTitle) {
      return {
        link: function($scope, $element, $attr) {
          if ($attr.idleDisabled) return;

          IdleTitle.store(true);

          $scope.$on('IdleWarn', function(e, countdown) {
            IdleTitle.setAsIdle(countdown);
          });
